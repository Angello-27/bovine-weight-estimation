...
class FarmModel(Document):
    ...
    name: Indexed(str) = Field(
        ..., description="Nombre de la finca (ej: Hacienda Gamelera)", min_length=1, max_length=200
    )
    owner_id: Indexed(UUID) = Field(
    ...
    capacity: int = Field(
        ..., description="Capacidad máxima de animales", ge=1
    )
