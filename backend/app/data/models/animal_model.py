"""
Animal Model - Beanie ODM (Data Layer)
...
from ...domain.shared.constants import AgeCategory
...
class AnimalModel(Document):
    ...
    ear_tag: Indexed(str, unique=True) = Field(  # type: ignore
        ..., description="Número de caravana/arete (único)"
    )
    breed: Indexed(str) = Field(..., description="Raza del animal (una de las 8)")  # type: ignore
    birth_date: datetime = Field(..., description="Fecha de nacimiento")
    gender: Indexed(str) = Field(..., description="Género: male o female")  # type: ignore
    ...
    # Metadata
    farm_id: UUID = Field(..., description="ID de la hacienda")  # type: ignore
