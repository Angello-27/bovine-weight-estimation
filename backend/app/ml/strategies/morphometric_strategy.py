...
class MorphometricWeightEstimationStrategy(BaseWeightEstimationStrategy):
    ...
    def estimate_weight(self, image_bytes: bytes, breed: BreedType) -> dict:
        ...
        params = self._breed_params.get(
            breed, {"a": 0.50, "b": 150, "min": 200, "max": 800}
        )
    ...
    def get_breed_coefficients(self) -> dict:
        ...
        """
        return {breed.value: params for breed, params in self._breed_params.items()}

    def update_breed_coefficients(
        self, breed: BreedType, a: float, b: float, min_weight: float, max_weight: float
    ) -> None:
        """
